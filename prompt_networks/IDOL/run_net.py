...
from detectron2.projects.idol.data import (
  ...
  COCO_CLIP_DatasetMapper,build_combined_loader
...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        #dataset_name = cfg.DATASETS.TRAIN[0]
            if dataset_name.startswith('coco'):
                mappers.append(COCO_CLIP_DatasetMapper(cfg, is_train=True))
            elif dataset_name.startswith('ytvis') or dataset_name.startswith('davis') or dataset_name.startswith('ovis'):
                mappers.append(YTVISDatasetMapper(cfg, is_train=True))
            else:
                raise NotImplementedError
        assert len(mappers) > 0, "No dataset is chosen!"
        if len(mappers) == 1:
            mapper,dataset_name = mappers[0], cfg.DATASETS.TRAIN[0]
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        elif dataset_name.startswith('ytvis') or dataset_name.startswith('davis') :
