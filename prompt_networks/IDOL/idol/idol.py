...
class IDOL(nn.Module):
    ...
    def inference(self, outputs, tracker, ori_size, image_sizes):
        ...
        for i_frame, (logits, output_boxes, output_embed) in enumerate(zip(
            vido_logits, video_output_boxes, video_output_embeds
         )):
            ...
            if i_frame>8:
                del_list = []
                for k,v in video_dict.items():
                    if v['boxes'][-1] is None and  v['boxes'][-2] is None and v['valid']<3:
                        del_list.append(k)   
        ...
        for inst_id,m in  enumerate(video_dict.keys()):
            ...
            #     else:
            #         #print(mask_i.shape, video_dict[m]['boxes'][n])
            ...
            for n in range(vid_len):
                ...
                if bbox_i is None:    
                    zero_mask = None # padding None instead of zero mask to save memory
                    boxes_list_i.append(zero_mask)
                else:
                    x, y, w, h = bbox_i
                    x1, y1 = max(torch.Tensor([0.]).cuda(), x - w / 2 ) * ori_size[1], max(torch.Tensor([0.]).cuda(), y - h / 2) * ori_size[0]
                    x2, y2 = min(torch.Tensor([1.]).cuda(), x + w / 2 ) * ori_size[1] , min(torch.Tensor([1.]).cuda(), y + h / 2) * ori_size[0]
                    ...
                    #pred_mask_i = pred_mask_i[:,:,:image_sizes[0],:image_sizes[1]] #crop the padding area
                    #pred_mask_i = (F.interpolate(pred_mask_i, size=(ori_size[0], ori_size[1]), mode='nearest')>0.5)[0,0].cpu() # resize to ori video size
                    boxes_list_i.append([x1.cpu().item(),y1.cpu().item(),x2.cpu().item(),y2.cpu().item()])
        ...
        if len(pred_cls) > 0:
            if self.is_multi_cls:
                ...
                out_bboxes = [boxes_list[valid_id] for valid_id in is_above_thres[0]]
            else:
            ...
            out_labels = []
            #out_masks = []
        #print(len(out_masks),len(out_bboxes))
        #print(out_masks[0], out_bboxes[0])
        video_output = {
            "image_size": ori_size,
